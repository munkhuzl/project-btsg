...
const RequestsList = () => {
    ...
    const handleStatusChange = async (id: string, status: 'pending' | 'accepted' | 'declined') => {
        try {
                ...
                <Card key={String(request._id)} className="p-6">
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 space-y-3">
                            {request.result && request.startTime && request.endTime && request.requestType && (
                                <div className="flex items-center gap-3">
                                    <h3 className="font-semibold text-gray-900">{request.firstname}</h3>
                                    <span>{getTypeLabel(request.requestType)}</span>
                                    <span
                                        className={`${getStatusColor(request.result)} px-3 py-1 rounded-md text-sm font-medium`}
                                    >
                  {request.result
                    ...
                    {request.startTime} – {request.endTime}
                                ...
                                {request.startTime && request.endTime && (<div className="flex items-center gap-2">
